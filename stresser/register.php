...
// Fix file permission issue for bus.txt
$dataFile = 'bus.txt';
$fileError = false;
...
} elseif (!is_writable($dataFile)) {
    if (!chmod($dataFile, 0666)) {
        $fileError = true;
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Check captcha first
    if (empty($user_captcha) || (int)$user_captcha !== $_SESSION['captcha']['a']) {
        ...
        // Proceed with registration validation
        if (empty($username) || empty($password) || empty($confirm)) {
            ...
            if (empty($errors)) {
                if ($fileError) {
                    $errors[] = "System error: Unable to write to data file. Please contact support.";
                } else {
                    // Format: username|password|0|0|free|30-12-2030|active|0
                    $userData = "$username|$password|0|0|free|30-12-2030|active|0\n";
                    if (file_put_contents($dataFile, $userData, FILE_APPEND)) {
                        ...
                        $errors[] = "Error creating account (write failed).";
                    }
                }
            }
        }
    ...
    // Regenerate captcha after processing (if errors, keep new one)
    if (!empty($errors)) {
        $num1 = rand(1, 15);
        $num2 = rand(1, 15);
        $_SESSION['captcha'] = ['q' => "$num1 + $num2", 'a' => $num1 + $num2];
...
<head>
    ...
    <title>Secure Registration | KillByte Solutions</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Main Layout - improved responsive */
        .auth-wrapper {
            position: relative;
            z-index: 10;
            width: 100%;
            max-width: 1000px; /* slightly smaller for compact luxury */
            display: flex;
            flex-wrap: wrap;
            align-items: stretch;
            justify-content: center;
            padding: 1.5rem;
            min-height: 100vh;
            gap: 2rem; /* fallback gap on mobile */
        }
        .auth-brand-col {
            flex: 1 1 350px;
            display: flex;
...
<body>
    ...
    <div class="auth-wrapper">
        ...
        <div class="auth-form-col">
            <h2 class="form-title">
                <span class="accent">Create</span> Account
            </h2>
            <form method="POST" id="registerForm">
                ...
                <div class="form-group">
                    <input type="password" name="confirm" id="confirmField" class="form-input" placeholder="Confirm Password" required>
